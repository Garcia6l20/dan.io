...
class Catch2(CMakeProject):
    ...
    cmake_config_definitions = {"CATCH_CONFIG_BAZEL_SUPPORT": "OFF"}
    cmake_options = {
        "android_logwrite": (
        ...
        "console_width": (
            ...
            80,
            "Read docs/configuration.md for details. Must form a valid integerliteral.",
        ),
        "counter": (
        ...
        "default_reporter": (
            ...
            "console",
            "Read docs/configuration.md for details. The name of thereporter should be without quotes.",
        ),
        "disable": (
...
def discover_tests(self, ExecutableClass):
    ...
    class Catch2Test(Test, ExecutableClass):
        ...
        async def __build__(self):
            if not super().up_to_date:
                await super().__build__()

            if self.output.exists():
                out, err, rc = await self.execute(
                    "--list-tests", no_raise=True, log=False, build=False
                )
                self.cases = list()
                ...
                async with aiofiles.open(filepath, "r") as f:
                    for lineno, line in enumerate(await f.readlines()):
                        match re_match(line):
                            case (
                                r"(TEST_CASE|SCENARIO|TEMPLATE_TEST_CASE)\(\s?\"(.*?)\".+" as m
                            ):
                                # macro = m[1]
                                name = m[2]
                                for case in self.cases:
                                    if case.name == name:
                                        ...
                                        break
                self.debug(
                    "test cases found: %s", ", ".join([c.name for c in self.cases])
                )
